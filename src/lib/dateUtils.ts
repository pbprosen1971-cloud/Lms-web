...
/**
 * Checks whether an upcoming exam has an explicit, valid scheduled live start time set by admin,
 ...
 *    Only timestamps with explicit hours and minutes (containing ':' and/or 'T') qualify.
 * 2. If no valid explicit time of day is specified, returns FALSE. The exam MUST remain upcoming until admin clicks "Make Live" or explicitly schedules a live datetime.
 * 3. If a valid schedule time is specified, returns TRUE only if Date.now() >= scheduledTimeMs.
 */
export function isScheduledLiveTimeReached(
  startTimeStr?: string, 
  examDateTimeStr?: string
): boolean {
  const timeCandidate = (startTimeStr && startTimeStr.trim()) || 
                        (examDateTimeStr && examDateTimeStr.trim()) || '';
  if (!timeCandidate || timeCandidate === 'নির্ধারিত নেই') return false;

  // Must have explicit time (hours and minutes), e.g. contains ':'
  if (!timeCandidate.includes(':')) {
    return false;
  }
...
export const formatBanglaDateTime = formatBengaliDateTimeSafe;
